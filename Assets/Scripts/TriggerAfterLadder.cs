...
public class TriggerAfterLadder : MonoBehaviour
{
    [SerializeField] private GameObject brotherGameObject;
    [SerializeField] private GameObject catToDisableGameObject;
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.TryGetComponent(out PlayerMovement playerMovement))
        {
            ...
            if (mouseLook)
            {
                ...
                if (pickUpObject && pickUpObject.name == "Ladder")
                {
                    HelpTheCat helpTheCat = FindAnyObjectByType<HelpTheCat>();
                    if (helpTheCat)
                    {
                        helpTheCat.StopMeowing();
                    }
                    
                    playerMovement.enabled = false;
                    mouseLook.enabled = false;
                    brotherGameObject.SetActive(true);
                    catToDisableGameObject.SetActive(false);
    ...
    private IEnumerator LerpRageBar()
    {
        ...
        {
            rageBarImage.fillAmount = Mathf.Lerp(rageBarImage.fillAmount, 0.33f, timer / duration);
            timer += Time.deltaTime;
            
            yield return null;
        }

        rageBarImage.fillAmount = 0.33f;
        yield return null;
    }
    ...
    private IEnumerator ZoomToBrother()
    {
        ...
        LevelManager.Instance.LoadScene(Scenes.Level2);
    }
}
